...
class Graph:
	...
	func add_edge(vertex_A, vertex_B, weight):
		...
		edge_weights[[vertex_A, vertex_B]] = weight
...
class MST:
	...
	func get_mst():
		...
		mst.add_vertex(graph.vertices[0])
		
		
		var vertices = [] + graph.vertices # copying vertices
		var edges = []
		
		# find lowest weight edge, add the point, remove that edge, repeat
...
class Dijkstra:
	...
	func find_shortest_path(vertexA:Vertex, vertexB:Vertex):
		var path = []
